      ...
      },
      {
        ...
        },
        {
          ...
          callback: function(e) {
            ...
            var insert_image = function(url, filename )
            {
                var html = '<img id="insertImageId" width="60px" height="60px" src="' + url + '" alt=""' + (filename ? ' title="'+filename.replace(/"/,'&quot;')+'"' : '') + ' />';
                
                //$("#editor1").innerHTML = html;
                e.$textarea.hide();
                //e.setContent(content + html);
                $("#testDiv").html(content + html);
            };
            ...
            var loadImageFromFile = function( file )
            {
                ...
                reader.onload = function(event) {
                    ...
                    insert_image(dataurl, file.name );
                };
        ...
        },
        {
            ...
            callback: function(e) {
              ...
              link = prompt('输入Word文件地址', 'http://');

              var urlRegex = new RegExp('^((http|https)://|(//))[a-z0-9]', 'i');
              if (link !== null && link !== '' && link !== 'http://' && urlRegex.test(link)) {
                var sanitizedLink = $('<div>' + link + '</div>').text();

                e.replaceSelection('![' + chunk + '](' + sanitizedLink + ' "' + '输入Word文件标题' + '")');
                cursor = selected.start + 2;

                e.setNextTab('在这里输入Word文件标题');
